...
static const std::map<std::string, SHELL_Cmd> shell_cmds = {
	...
	// a shell command, not a Z: program: WORKBENCH.COM does not fit 8.3
	{ "WORKBENCH", {&DOS_Shell::CMD_WORKBENCH, "WORKBENCH", HELP_Filter::Common, HELP_Category::Dosbox } }
};
// clang-format on
...
bool DOS_Shell::ExecuteConfigChange(const char* const property_name,
                                    const std::string _args)
{
	const auto section_dosbox = get_section("dosbox");
		...
		for (const auto& s : HELP_GetHelpList()) {
			...
			if (!category_started) {
				...
				output.AddString(format_header,
				                 HELP_CategoryHeading(cat).c_str());
...
char* format_date(const uint16_t year, const uint8_t month, const uint8_t day)
{
	...
	case DosDateFormat::DayMonthYear:
		result = safe_sprintf(format_string, "D%cM%cY", date_separator, date_separator);
		break;
	case DosDateFormat::YearMonthDay:
		result = safe_sprintf(format_string, "Y%cM%cD", date_separator, date_separator);
		break;
	default: // DosDateFormat::MonthDayYear
		result = safe_sprintf(format_string, "M%cD%cY", date_separator, date_separator);
	}
	if (result < 0) {
...
void DOS_Shell::CMD_TIME(char* args)
{
	...
	if (scan_and_remove_cmdline_switch(args, "?")) {
		...
		const auto example = format_str("13%c14%c15",
		                                time_separator,
		                                time_separator);
...
void DOS_Shell::CMD_CHOICE(char* args)
{
	...
	bool do_timeout = has_default && contains(choices, default_choice) &&
	                  default_wait_s > 0;
	...
	while (!contains(choices, choice)) {
		if (do_timeout) {
			choice = read_from_keyboard_with_timeout(default_wait_s * 1000,
			                                         default_choice);
		} else {
...
void DOS_Shell::CMD_WORKBENCH(char* args)
{
	HELP("WORKBENCH");
	...
	case WebserverToolLaunch::ApiOff:
		WriteOut(MSG_Get("SHELL_CMD_WORKBENCH_API_OFF"));
		return;
	case WebserverToolLaunch::BrowserFailed:
		WriteOut(MSG_Get("SHELL_CMD_WORKBENCH_BROWSER_FAILED"),
		         result.url.c_str());
		return;
	case WebserverToolLaunch::Opened:
		WriteOut(MSG_Get("SHELL_CMD_WORKBENCH_OPENED"), result.url.c_str());
		break;
	case WebserverToolLaunch::AlreadyOpen:
		WriteOut(MSG_Get("SHELL_CMD_WORKBENCH_ALREADY_OPEN"),
		         result.url.c_str());
		return;
	}

	const auto endpoint = WEBSERVER_GetEndpoint();
	if (endpoint && !WEBSERVER_ToolPageIsLoopback(*endpoint)) {
		WriteOut(MSG_Get("SHELL_CMD_WORKBENCH_TOKEN_PROMPT"));
	}
}
