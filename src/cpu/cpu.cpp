...
enum TSwitchType { TSwitch_JMP, TSwitch_CALL_INT, TSwitch_IRET };
...
static bool cpu_switch_task(Bitu new_tss_selector, TSwitchType tstype, Bitu old_eip)
{
	...
	if (!new_tss.SetSelector(new_tss_selector)) {
		E_Exit("Illegal TSS for switch, selector=%" sBitfs(x) ", switchtype=%x",
		       new_tss_selector,
		       tstype);
	}
	...
	if (reg_flags & FLAG_VM) {
		...
		case DESC_CODE_R_NC_A:
		case DESC_CODE_R_NC_NA:
			if (cpu.cpl != cs_desc.DPL()) {
				E_Exit("Task CS RPL != DPL");
			}
			goto doconforming;
	...
	if (!cpu_tss.SetSelector(new_tss_selector)) {
		LOG(LOG_CPU, LOG_NORMAL)("TaskSwitch: set tss selector %X failed",
		                         new_tss_selector);
	}
	//	cpu_tss.desc.SetBusy(true);
	//	cpu_tss.SaveSelector();
	//	LOG_MSG("Task CPL %X CS:%X IP:%X SS:%X SP:%X eflags
...
//		E_Exit("Call to interrupt 0xCD this is BAD");
	...
	if (!cpu.pmode) {
		...
		if ((reg_flags & FLAG_VM) && (type & CPU_INT_SOFTWARE) &&
		    !(type & CPU_INT_NOIOPLCHECK)) {
			//			LOG_MSG("Software int in v86, AH
			//%X IOPL %x",reg_ah,(reg_flags & FLAG_IOPL) >>12);
		...
		case DESC_386_TRAP_GATE: {
			...
			case DESC_CODE_R_NC_NA:
				if (cs_dpl < cpu.cpl) {
					...
					CPU_CHECK_COND(!cs_desc.saved.seg.p,
					               ...
					               EXCEPTION_NP,
					               (gate_sel & 0xfffc) +
					                       ((type & CPU_INT_SOFTWARE)
					                                ? 0
					                                : 1))
					CPU_CHECK_COND((reg_flags & FLAG_VM) &&
					                       (cs_dpl != 0),
					...
					CPU_CHECK_COND(
					        ...
					        EXCEPTION_TS,
					        (n_ss & 0xfffc) + ((type & CPU_INT_SOFTWARE)
					                                   ? 0
					                                   : 1))
					CPU_CHECK_COND(
					        ((n_ss & 3) != cs_dpl) ||
					                (n_ss_desc.DPL() != cs_dpl),
					...
					// check if stack segment is a writable
					// data segment
					switch (n_ss_desc.Type()) {
					case DESC_DATA_EU_RW_NA:
					case DESC_DATA_EU_RW_A:
					case DESC_DATA_ED_RW_NA:
					case DESC_DATA_ED_RW_A: break;
					default:
					...
					if (gate.Type() & 0x8) { /* 32-bit Gate */
						...
						if (reg_flags & FLAG_VM) {
							E_Exit("V86 to 16-bit gate");
						}
						CPU_Push16(o_ss);
						CPU_Push16(o_esp);
					}
					//						LOG_MSG("INT:Gate
					// to inner level SS:%X SP:%X",n_ss,n_esp);
					goto do_interrupt;
				}
				if (cs_dpl != cpu.cpl) {
			...
			SETFLAGBIT(NT, false);
			SETFLAGBIT(VM, false);
			LOG(LOG_CPU, LOG_NORMAL)("INT:Gate to %X:%X big %d %s",
			                         gate_sel,
			                         gate_off,
			                         cs_desc.Big(),
			                         gate.Type() & 0x8 ? "386" : "286");
			return;
		}
		case DESC_TASK_GATE:
			CPU_CHECK_COND(!gate.saved.seg.p,
	...
	if (!cpu.pmode) { /* RealMode IRET */
		...
		if (reg_flags & FLAG_VM) {
			if ((reg_flags & FLAG_IOPL) != FLAG_IOPL) {
				...
				if (use32) {
					uint32_t new_eip = mem_readd(
					        SegPhys(ss) +
					        (reg_esp & cpu.stack.mask));
					uint32_t tempesp = (reg_esp &
					                    cpu.stack.notmask) |
					                   ((reg_esp + 4) &
					                    cpu.stack.mask);
					uint32_t new_cs = mem_readd(
					        SegPhys(ss) +
					        (tempesp & cpu.stack.mask));
					tempesp = (tempesp & cpu.stack.notmask) |
					          ((tempesp + 4) & cpu.stack.mask);
					uint32_t new_flags = mem_readd(
					        SegPhys(ss) +
					        (tempesp & cpu.stack.mask));
					reg_esp = (tempesp & cpu.stack.notmask) |
					          ((tempesp + 4) & cpu.stack.mask);

					reg_eip = new_eip;
					SegSet16(cs, (uint16_t)(new_cs & 0xffff));
					/* IOPL can not be modified in v86 mode
					 * by IRET */
					CPU_SetFlags(new_flags,
					             FMASK_NORMAL | FLAG_NT);
				} else {
					uint16_t new_eip = mem_readw(
					        SegPhys(ss) +
					        (reg_esp & cpu.stack.mask));
					uint32_t tempesp = (reg_esp &
					                    cpu.stack.notmask) |
					                   ((reg_esp + 2) &
					                    cpu.stack.mask);
					uint16_t new_cs = mem_readw(
					        SegPhys(ss) +
					        (tempesp & cpu.stack.mask));
					tempesp = (tempesp & cpu.stack.notmask) |
					          ((tempesp + 2) & cpu.stack.mask);
					uint16_t new_flags = mem_readw(
					        SegPhys(ss) +
					        (tempesp & cpu.stack.mask));
					reg_esp = (tempesp & cpu.stack.notmask) |
					          ((tempesp + 2) & cpu.stack.mask);
		...
		if (use32) {
			...
			tempesp = (reg_esp & cpu.stack.notmask) |
			          ((reg_esp + 4) & cpu.stack.mask);
			n_cs_sel = mem_readd(SegPhys(ss) +
			                     (tempesp & cpu.stack.mask)) &
			           0xffff;
			tempesp = (tempesp & cpu.stack.notmask) |
			          ((tempesp + 4) & cpu.stack.mask);
			...
			if ((n_flags & FLAG_VM) && (cpu.cpl == 0)) {
				...
				LOG(LOG_CPU, LOG_NORMAL)("IRET:Back to V86: CS:%X IP %X SS:%X SP %X FLAGS:%X",
				                         SegValue(cs),
				                         reg_eip,
				                         SegValue(ss),
				                         reg_esp,
			...
			if (n_flags & FLAG_VM) {
				E_Exit("IRET from pmode to v86 with CPL!=0");
			}
		} else {
		...
		if (n_cs_rpl == cpu.cpl) {
			...
			DestroyConditionFlags();
			LOG(LOG_CPU, LOG_NORMAL)("IRET:Same level:%X:%X big %d",
			                         n_cs_sel,
			                         n_eip,
			                         cpu.code.big);
		} else {
			...
			Bitu n_ss, n_esp;
			if (use32) {
				n_esp   = mem_readd(SegPhys(ss) +
                                                  (tempesp & cpu.stack.mask));
				tempesp = (tempesp & cpu.stack.notmask) |
				          ((tempesp + 4) & cpu.stack.mask);
				n_ss = mem_readd(SegPhys(ss) +
				                 (tempesp & cpu.stack.mask)) &
				       0xffff;
			} else {
				n_esp   = mem_readw(SegPhys(ss) +
                                                  (tempesp & cpu.stack.mask));
				tempesp = (tempesp & cpu.stack.notmask) |
				          ((tempesp + 2) & cpu.stack.mask);
				n_ss = mem_readw(SegPhys(ss) +
				                 (tempesp & cpu.stack.mask));
			}
			CPU_CHECK_COND((n_ss & 0xfffc) == 0,
			...
			LOG(LOG_CPU, LOG_NORMAL)("IRET:Outer level:%X:%X big %d",
			                         n_cs_sel,
			                         n_eip,
			                         cpu.code.big);
		}
	...
	if (!cpu.pmode || (reg_flags & FLAG_VM)) {
		...
		case DESC_CODE_R_NC_NA:
			...
			CPU_CHECK_COND(cpu.cpl != desc.DPL(),
			               ...
			               EXCEPTION_GP,
			               selector & 0xfffc)
			LOG(LOG_CPU, LOG_NORMAL)("JMP:Code:NC to %X:%X big %d",
			                         selector,
			                         offset,
			                         desc.Big());
			goto CODE_jmp;
		...
		case DESC_CODE_R_C_A:
		case DESC_CODE_R_C_NA:
			LOG(LOG_CPU, LOG_NORMAL)("JMP:Code:C to %X:%X big %d",
			                         selector,
			                         offset,
			                         desc.Big());
			CPU_CHECK_COND(cpu.cpl < desc.DPL(),
	...
	if (!cpu.pmode || (reg_flags & FLAG_VM)) {
		...
		case DESC_CODE_R_NC_NA:
			...
			CPU_CHECK_COND(call.DPL() != cpu.cpl,
			               ...
			               selector & 0xfffc)
			LOG(LOG_CPU,
			    LOG_NORMAL)("CALL:CODE:NC to %X:%X", selector, offset);
			goto call_code;
		case DESC_CODE_N_C_A:
		case DESC_CODE_N_C_NA:
		case DESC_CODE_R_C_A:
		case DESC_CODE_R_C_NA:
			CPU_CHECK_COND(call.DPL() > cpu.cpl,
			               ...
			               selector & 0xfffc)
			LOG(LOG_CPU,
			    LOG_NORMAL)("CALL:CODE:C to %X:%X", selector, offset);
		call_code:
			if (!call.saved.seg.p) {
		...
		case DESC_286_CALL_GATE: {
			...
			case DESC_CODE_R_NC_NA:
				...
				if (n_cs_dpl < cpu.cpl) {
					...
					CPU_CHECK_COND((n_ss_sel & 0xfffc) == 0,
					               ...
					               EXCEPTION_TS,
					               0)
					CPU_CHECK_COND(!cpu.gdt.GetDescriptor(n_ss_sel,
					                                      n_ss_desc),
					               "CALL:Gate:Invalid SS selector",
					               EXCEPTION_TS,
					               n_ss_sel & 0xfffc)
					CPU_CHECK_COND(((n_ss_sel & 3) !=
					                n_cs_desc.DPL()) ||
					                       (n_ss_desc.DPL() !=
					                        n_cs_desc.DPL()),
					...
					E_Exit("CALL:GATE:CS DPL>CPL"); // or
					                                // #GP(sel)
				}
				[[fallthrough]];
			case DESC_CODE_N_C_A:
			...
			case DESC_CODE_R_C_NA:
				...
				break;
			default: E_Exit("CALL:GATE:CS no executable segment");
			}
		} /* Call Gates */
		break;
		case DESC_386_TSS_A:
...
void CPU_RET(bool use32, Bitu bytes, [[maybe_unused]] Bitu oldeip)
{
	if (!cpu.pmode || (reg_flags & FLAG_VM)) {
		...
		if (!use32) {
			selector = mem_readw(SegPhys(ss) +
			                     (reg_esp & cpu.stack.mask) + 2);
		} else {
			selector = mem_readd(SegPhys(ss) +
			                     (reg_esp & cpu.stack.mask) + 4) &
			           0xffff;
		}
		...
		RET_same_level:
			...
			LOG(LOG_CPU, LOG_NORMAL)("RET - Same level to %X:%X RPL %X DPL %X",
			                         selector,
			                         offset,
			                         rpl,
			                         desc.DPL());
			...
			//			LOG(LOG_MISC,LOG_ERROR)("RET -
			// Higher level to %X:%X RPL %X DPL
			//%X",selector,offset,rpl,desc.DPL());
			return;
		}
...
bool CPU_LTR(Bitu selector)
{
	...
	if ((desc.Type() == DESC_286_TSS_A) || (desc.Type() == DESC_386_TSS_A)) {
		if (!desc.saved.seg.p) {
			LOG(LOG_CPU, LOG_ERROR)("LTR failed, selector=%X (not present)",
			                        selector);
			return CPU_PrepareException(EXCEPTION_NP, selector);
		}
		if (!cpu_tss.SetSelector(selector)) {
			E_Exit("LTR failed, selector=%" sBitfs(X), selector);
		}
		cpu_tss.desc.SetBusy(true);
		cpu_tss.SaveSelector();
	} else {
		/* Descriptor was no available TSS descriptor */
		LOG(LOG_CPU, LOG_NORMAL)("LTR failed, selector=%X (type=%X)",
		                         selector,
		                         desc.Type());
		return CPU_PrepareException(EXCEPTION_GP, selector);
	}
...
Bitu CPU_GET_CRX(Bitu cr)
{
	...
	case 0:
		if (CPU_ArchitectureType >= ArchitectureType::Pentium) {
			...
			return (cpu.cr0 & 0xe005003f);
		} else {
			return (cpu.cr0 | 0x7ffffff0);
		}
	case 2: return paging.cr2;
	case 3: return PAGING_GetDirBase() & 0xfffff000;
	default: LOG(LOG_CPU, LOG_ERROR)("Unhandled MOV XXX, CR%d", cr); break;
	}
	return 0;
}
...
bool CPU_WRITE_DRX(Bitu dr, Bitu value)
{
	...
	case 1:
	case 2:
	case 3: cpu.drx[dr] = value; break;
	case 4:
	case 6: cpu.drx[6] = (value | 0xffff0ff0) & 0xffffefff; break;
	case 5:
	case 7:
		if (CPU_ArchitectureType < ArchitectureType::Pentium) {
...
bool CPU_READ_DRX(Bitu dr, uint32_t& retvalue)
{
	...
	case 3:
	case 6:
	case 7: retvalue = cpu.drx[dr]; break;
	case 4: retvalue = cpu.drx[6]; break;
	case 5: retvalue = cpu.drx[7]; break;
	default:
...
bool CPU_WRITE_TRX(Bitu tr, Bitu value)
{
	...
	switch (tr) {
		//	case 3:
	case 6:
	case 7: cpu.trx[tr] = value; return false;
	default:
...
bool CPU_READ_TRX(Bitu tr, uint32_t& retvalue)
{
	...
	switch (tr) {
		//	case 3:
	case 6:
	case 7: retvalue = cpu.trx[tr]; return false;
	default: LOG(LOG_CPU, LOG_ERROR)("Unhandled MOV XXX, TR%d", tr); break;
	}
	return CPU_PrepareException(EXCEPTION_UD, 0);
}
...
void CPU_VERR(Bitu selector)
{
	...
	case DESC_CODE_R_NC_NA:
		...
		break;
	default: SETFLAGBIT(ZF, false); return;
	}
	SETFLAGBIT(ZF, true);
}
	...
	if (!cpu.pmode || (reg_flags & FLAG_VM)) {
		...
		if (seg == ss) {
			...
			if (!cpu.gdt.GetDescriptor(value, desc)) {
				//				E_Exit("CPU_SetSegGeneral:
				// Stack segment beyond limits");
				return CPU_PrepareException(EXCEPTION_GP,
				                            value & 0xfffc);
			}
			if (((value & 3) != cpu.cpl) || (desc.DPL() != cpu.cpl)) {
				//				E_Exit("CPU_SetSegGeneral:
				// Stack segment with invalid privileges");
				return CPU_PrepareException(EXCEPTION_GP,
				                            value & 0xfffc);
			}
			...
			if (!desc.saved.seg.p) {
				//				E_Exit("CPU_SetSegGeneral:
				// Stack segment not present");	// or #SS(sel)
				return CPU_PrepareException(EXCEPTION_SS,
				                            value & 0xfffc);
			}
			...
			if (!desc.saved.seg.p) {
				// win
				return CPU_PrepareException(EXCEPTION_NP,
				                            value & 0xfffc);
			}
...
bool CPU_PopSeg(SegNames seg, bool use32)
{
	...
	uint32_t new_esp = (reg_esp & cpu.stack.notmask) |
	                   ((reg_esp + addsp) & cpu.stack.mask);
...
bool CPU_CPUID()
{
	...
	case 1: // Get processor type/family/model/stepping and feature flags
		if ((CPU_ArchitectureType == ArchitectureType::Intel486NewSlow) ||
			...
			reg_ebx = 0;          // Not supported
			reg_ecx = 0;          // No features
			reg_edx = 0x00800011; // FPU + Time Stamp Counter
			                      // (RDTSC) + MMX
		} else {
			return false;
		}
	...
	if (!use32) {
		...
		if (level) {
			for (Bitu i = 1; i < level; i++) {
				sp_index -= 2;
				bp_index -= 2;
				mem_writew(SegPhys(ss) + sp_index,
				           mem_readw(SegPhys(ss) + bp_index));
			}
			sp_index -= 2;
			mem_writew(SegPhys(ss) + sp_index, reg_bp);
		}
		...
		if (level) {
			for (Bitu i = 1; i < level; i++) {
				sp_index -= 4;
				bp_index -= 4;
				mem_writed(SegPhys(ss) + sp_index,
				           mem_readd(SegPhys(ss) + bp_index));
			}
			sp_index -= 4;
			mem_writed(SegPhys(ss) + sp_index, reg_ebp);
		}
...
public:
	...
	void ConfigureCyclesModern(SectionProp* secprop)
	{
		...
		auto clamp_and_sync_cycles = [](const int cycles,
