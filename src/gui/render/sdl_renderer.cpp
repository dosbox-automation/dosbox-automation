        ...
        : texture_filter_mode(texture_filter_mode)
{
	...
	// --parent-window embedding. SDL3 dropped SDL_CreateWindowFrom; the
	// foreign handle goes in as a platform-specific creation property, so
	// the active video driver decides which property applies.
	const auto& parent_window_opt = control->arguments.parent_window;
	if (parent_window_opt) {
		const char* video_driver = SDL_GetCurrentVideoDriver();
		const std::string_view driver = video_driver ? video_driver : "";

		if (driver == "x11") {
			if (*parent_window_opt > 0xFFFFFFFFull) {
				// X11 window ids are 32-bit XIDs
				LOG_WARNING("SDL: Parent window id 0x%llx is not a valid X11 window - starting standalone",
				            static_cast<unsigned long long>(*parent_window_opt));
			} else {
				SDL_SetNumberProperty(props,
				                      SDL_PROP_WINDOW_CREATE_X11_WINDOW_NUMBER,
				                      static_cast<Sint64>(*parent_window_opt));
				window = SDL_CreateWindowWithProperties(props);
				if (!window) {
					LOG_WARNING("SDL: Failed to embed in parent window 0x%llx: %s - falling back to standalone",
					            static_cast<unsigned long long>(*parent_window_opt),
					            SDL_GetError());
					SDL_ClearProperty(props, SDL_PROP_WINDOW_CREATE_X11_WINDOW_NUMBER);
				}
			}
#ifdef WIN32
			...
			SDL_SetPointerProperty(props,
			                       ...
			                       reinterpret_cast<void*>(static_cast<uintptr_t>(*parent_window_opt)));
			window = SDL_CreateWindowWithProperties(props);
			if (!window) {
				LOG_WARNING("SDL: Failed to embed in parent window 0x%llx: %s - falling back to standalone",
				            static_cast<unsigned long long>(*parent_window_opt),
				            SDL_GetError());
				SDL_ClearProperty(props, SDL_PROP_WINDOW_CREATE_WIN32_HWND_POINTER);
			}
#endif
		} else {
			LOG_WARNING("SDL: --parent-window is not supported on video driver '%s' - starting standalone",
