...
//   complex) - it calls 'TTF_DrawPrepareScreen()' at the beginning of the
//   frame to fetch the color palette, than 'TTF_DrawPrepareBlockLine()' at
//   the beginning of each row of text to interpret the character attributes,
//   and then 'TTF_DrawLine()' to render the concrete line.
//
...
//   'TTF_ShouldChangeScreenOverride()' calls.
...
static uint8_t* VGA_Draw_CGA16_Line(Bitu vidstart, Bitu line)
{
	...
	for (uint32_t i = 2, j = 0, x = 0; x < vga.draw.blocks * 2;
		...
		// colors are OR'd on top of this
		constexpr uint32_t foundation = 0xc0708030;

		const uint32_t pixel = temp[i] | (temp[i + 1] << 8) |
...
static uint8_t* Composite_Process(uint8_t border, uint32_t blocks, bool double_width)
{
	...
	if (vga.tandy.mode.is_black_and_white_mode) {
		...
		for (int x = -1; x < w + 1; ++x) {
			ap[x] = i[-4] -
			        left_shift_signed(i[-2] - i[0] + i[2], 1) + i[4];
...
static uint8_t* draw_unwrapped_line_from_dac_palette_with_hwcursor(Bitu vidstart,
                                                                   Bitu /*line*/)
{
	...
	// Draw mouse cursor
	// ~~~~~~~~~~~~~~~~~
	// The cursor is a 64x64 pattern which is shifted (inside the 64x64
	// mouse cursor space) to the right by posx pixels and up by posy
	// pixels.
	//
	// This is used when the mouse cursor partially leaves the screen. It is
	// arranged as bitmap of 16bits of bit A followed by 16bits of bit B,
	// each AB bits corresponding to a cursor pixel. The whole map is 8kB in
	// size.
	...
	for (auto m = mem_start; m < mem_end; m += mem_delta[m & 1]) {
		...
		while (cursor_bit != 0) {
			...
			case CursorOp::Foreground:
				*cursor_addr = fg_colour;
				break;
			case CursorOp::Background:
				*cursor_addr = bg_colour;
				break;
...
static uint8_t* VGA_Draw_LIN16_Line_HWMouse(Bitu vidstart, Bitu /*line*/)
{
	...
	if ((vga.s3.hgc.posx >= vga.draw.image_info.width) ||
		...
		Bitu sourceStartBit = ((lineat - vga.s3.hgc.originy) +
		                       vga.s3.hgc.posy) *
		                              64 +
		                      vga.s3.hgc.posx;

		Bitu cursorMemStart = ((sourceStartBit >> 2) & ~1) +
		                      (((uint32_t)vga.s3.hgc.startaddr) << 10);
		...
		for (Bitu m = cursorMemStart; m < cursorMemEnd;
			...
			for (uint8_t bit = (0x80 >> cursorStartBit); bit != 0;
			     bit >>= 1) {

				// For each bit
				cursorStartBit = 0;
				if (bitsA & bit) {
					// Byte order doesn't matter here as all
					// bits get flipped
					if (bitsB & bit) {
						const auto xat = read_unaligned_uint16_at(
						        TempLine, i);

						write_unaligned_uint16_at(
						        TempLine, i, xat ^ 0xffff);
					}
					// else Transparent
					//
				} else if (bitsB & bit) {
					// Source as well as destination are
					// uint8_t arrays, so this should work
					// out endian-wise?
					const auto fore = read_unaligned_uint16(
					        vga.s3.hgc.forestack);

					write_unaligned_uint16_at(TempLine, i, fore);

				} else {
					const auto back = read_unaligned_uint16(
					        vga.s3.hgc.backstack);
	...
	if ((vga.s3.hgc.posx >= vga.draw.image_info.width) ||
		...
		for (Bitu m = cursorMemStart; m < cursorMemEnd;
			...
			for (uint8_t bit = (0x80 >> cursorStartBit); bit != 0;
				...
				if (bitsA & bit) {
					if (bitsB & bit) {
						const auto xat = read_unaligned_uint32_at(
						        TempLine, i);
					...
					const auto fore = read_unaligned_uint32(
					        vga.s3.hgc.forestack);

					write_unaligned_uint32_at(TempLine, i, fore);

				} else {
					const auto back = read_unaligned_uint32(
					        vga.s3.hgc.backstack);
...
static const uint8_t* VGA_Text_Memwrap(Bitu vidstart)
{
	...
	if ((vidstart + line_end) > vga.draw.linear_mask) {
		...
		// need a temporary storage - TempLine/2 is ok for a bit more
		// than 132 columns
...
static uint8_t* VGA_TEXT_Draw_Line(Bitu vidstart, Bitu line)
{
	...
	if (font_addr < vga.draw.blocks) {
		...
		uint32_t att =
		        TXT_FG_Table[vga.tandy.draw_base[vga.draw.cursor.address + 1] & 0xf];
...
static uint8_t* draw_text_line_from_dac_palette(Bitu vidstart, Bitu line)
{
	...
	if (vga.draw.panning) {
		// If the text is panned part of an additional character becomes
		// visible
		++blocks;
	}
	...
	if (!SkipCursor(vidstart, line)) {
		// The adress of the attribute that makes up the cell the cursor
		// is in
		...
		if (attr_addr < vga.draw.blocks) {
			...
			// The cursor block's byte-offset into the rendering
			// buffer.
			const auto cursor_draw_offset = check_cast<uint16_t>(
			        attr_addr * vga.draw.pixels_per_character *
			        bytes_per_pixel);
...
static void VGA_DrawSingleLine([[maybe_unused]] uint32_t dummy)
{
	...
	if (vga.draw.lines_done < vga.draw.lines_total) {
		...
		if (TTF_ShouldChangeScreenOverride()) {
			...
			// We've drawn the full frame, notify the renderer the
			// frame is ready
...
{
	auto skip_cursor = [&](const uint32_t line_in_block,
	                       const uint32_t cursor_block) {
		if (cursor_block >= vga.draw.ttf.blocks_horizontal) {
			return true;
		}

		return SkipCursor(vga.draw.address, TTF_HardwareLine(line_in_block));
	};

	if (vga.draw.ttf.render_lines_done == 0) {
		// Start of the screen
		TTF_DrawPrepareScreen();
	}
	...
	while (lines--) {
		...
		if (vga.draw.address_line == 0) {
			...
			for (uint32_t line_in_block = 0;
			     line_in_block < vga.draw.ttf.block_height;
			     line_in_block++) {
				...
				if (skip_cursor(line_in_block, cursor_block)) {
					...
					const auto color_index =
						TXT_FG_Table[vga.tandy.draw_base[vga.draw.cursor.address + 1] & 0xf];
					const auto& cursor_color =
					        render.palette.rgb[color_index % 0x100];
...
static void VGA_VerticalTimer(uint32_t /*val*/)
{
	...
	// go figure...
	if (is_machine_ega()) {
		if (vga.draw.image_info.double_height) {
			// Spacepigs EGA Megademo
	...
	case M_HERC_TEXT:
		...
		// If blinking is enabled, 'blink' will toggle between true and
	...
	if (vga.draw.vblank_skip) {
		draw_skip = vga.draw.delay.htotal *
		            static_cast<double>(vga.draw.vblank_skip);
...
void VGA_CheckScanLength(void)
{
	switch (vga.mode) {
	case M_EGA:
	case M_LIN4: vga.draw.address_add = vga.config.scan_len * 16; break;
	...
	case M_LIN16:
	case M_LIN24:
	case M_LIN32: vga.draw.address_add = vga.config.scan_len * 8; break;

	case M_TEXT: vga.draw.address_add = vga.config.scan_len * 4; break;
	...
	case M_TANDY16: vga.draw.address_add = vga.draw.blocks; break;

	case M_TANDY_TEXT:
	case M_CGA_TEXT_COMPOSITE:
	case M_HERC_TEXT: vga.draw.address_add = vga.draw.blocks * 2; break;

	case M_HERC_GFX: vga.draw.address_add = vga.draw.blocks; break;

	default: vga.draw.address_add = vga.draw.blocks * 8; break;
	}
}
                                                          ...
                                                          const uint32_t vtotal)
{
	...
	default:
		...
		// removed in the future. But it should make much of a difference
		// in the grand scheme of things, so leaving it alone for now.
		//
...
static VgaTimings calculate_vga_timings()
{
	...
	if (is_machine_ega_or_better()) {
		...
		if (is_machine_vga_or_better()) {
			...
			horiz.blanking_end |= (vga.crtc.end_horizontal_retrace &
			                       0x80) >>
			                      2;
			...
			vert.blanking_start |= vga.crtc.maximum_scan_line.start_vertical_blanking_bit9
			                    << 4;
		...
		// Most graphics cards agree that lines zero to vertical
		// blanking end are blanked. Tsend ET4000 doesn't blank at all
		// if vert.blanking_start == vert.blanking_end. ET3000 blanks
		// lines 1 to vert.blanking_end (255/6 lines).
		if (vert.blanking_start != 0) {
			vert.blanking_start += 1;

			vert.blanking_end = (vert.blanking_end - vert.blanking_start) &
			                    0x7f;

			if (!vert.blanking_end) {
				vert.blanking_end = vert.blanking_start + 0x7f + 1;
			} else {
				vert.blanking_end = vert.blanking_start +
				                    vert.blanking_end;
			}
		}
		vert.blanking_end++;
		...
		if (svga.get_clock) {
			...
			case 0:
				clock = is_machine_ega() ? CgaPixelClockHz
				                         : Vga640PixelClockHz;
				break;
			case 1:

			default:
				clock = is_machine_ega() ? EgaPixelClockHz
				                         : Vga720PixelClockHz;
				break;
			}
		}
		...
		// vsync width is fixed to 16 lines on the MC6845 TODO Tandy
		vert.retrace_end = vert.retrace_start + 16;
	...
	if (is_machine_vga_or_better()) {
		...
		if (vert.blanking_start < vert.total) {
			...
			if (vert.blanking_end > vert.total) {
				...
				// On blanking wrap to 0, the first line is not
				// blanked this is used by the S3 BIOS and other
				// S3 drivers in some SVGA modes
				...
				LOG(LOG_VGA, LOG_WARN)("Blanking wrap to line %u",
				                       vblank_skip);

			} else if (vert.blanking_start <= 1) {
				// Blanking is used to cut lines at the start of
				// the screen
				vblank_skip = vert.blanking_end;
				LOG(LOG_VGA, LOG_WARN)("Upper %u lines of the screen blanked",
				                       vblank_skip);

			} else if (vert.blanking_start < vert.display_end) {
				if (vert.blanking_end < vert.display_end) {
					// The game wants a black bar somewhere
					// on the screen
					LOG(LOG_VGA,
					    LOG_WARN)("Unsupported blanking: line %u-%u",
					              vert.blanking_start,
					              vert.blanking_end);
				} else {
...
ImageInfo setup_drawing()
{
	...
	case MachineType::Vga:
		if (is_high_resolution_mode(CurMode)) {
			...
			// Per-scanline rendering for accurate mid-frame
			// VGA register changes (raster effects, palette
			// changes in games like Lemmings and Pinball Dreams).
			vga.draw.mode = DrawMode::Scanline;
		}
		break;
	...
	// scan doubling only.
	if (is_machine_ega_or_better()) {
		vga.draw.address_line_total = vga.crtc.maximum_scan_line.maximum_scan_line + 1;
	} else {
	...
	// Force VRAM font check - might be needed in certain situations, like when
	// switching from 8x16 to 8x14 EGA/VGA font
	vga.draw.ttf.vram_font_dirty_flag = true;
	// Check if we are going to use a TTF output
	vga.draw.ttf.override = TTF_ShouldOverrideScreen();
	if (vga.draw.ttf.override) {
		// Similar to 'DrawMode::Part', draws one row of text a time.
		// With the TrueType renderer changing the VGA registers mid
		// frame won't work correctly nevertheless, so no point in using
		// anything similar to 'DrawMode::Scanline'.
	...
	auto pcjr_or_tga = [=]() {
		return is_machine_pcjr() ? GraphicsStandard::Pcjr
	...
	// The result of this should be rendering a short (by height) image in
	// the horizonal center with black on the top/bottom. Use this
	// hard-coded value when calculating pixel aspect ratio so this effect
	// looks correct.
	...
	case M_LIN32: {
		...
		case M_LIN8:
			...
			if (is_pixel_doubling || (svga_type == SvgaType::S3 &&
			                          !(vga.s3.reg_3a & 0x10))) {
		...
		// than 13h (and its tweak-mode variants; we'll just fake it
		// with `double_height`.
		if (is_vga_scan_doubling_bit_set()) {
	...
	case M_VGA: {
		...
		if (video_mode.is_double_scanned_mode) {
			...
			// Some rare demos set up odd Maximum Scan Line CRTC
			// register values; for example, Show by Majic 12 uses
			// the value 4 during the zoom-rotator part in the intro
			// to set up "scanline quintupling". That's right, every
			// scanline is repeated 4 times, resulting in a total
			// number of 5 scanlines per "logical pixel"!
			//
			// We're forcing such scanline repeating even in forced
			// single scan mode to yield correct results.
			const auto is_odd_address_line_total = vga.draw.address_line_total &
			                                       1;

			if (vga.draw.scan_doubling_allowed ||
			    is_odd_address_line_total) {
				vga.draw.is_double_scanning = true;
	...
	case M_EGA:
		...
		if (is_machine_vga_or_better()) {
			...
			// literally render twice as many rows) for the M_VGA
			// modes and M_EGA modes on emulated VGA adapters only;
			// for everything else, we "fake double-scan" on VGA
			// (render single-scanned, then double the image
			// vertically with a scaler).
			//
	...
	if (is_machine_vga_or_better() && (vga.mode == M_CGA2 || vga.mode == M_CGA4)) {
		vblank_skip /= 2;
	}
	...
	if (vga.draw.ttf.override) {
		...
		case AspectRatioCorrectionMode::SquarePixels:
			// Override PARs if square pixels are forced in aspect
			// ratio correction disabled mode
		...
		case AspectRatioCorrectionMode::Stretch: {
			// Stretch image to the viewport and calculate the
			// resulting PARs
			const auto viewport_px = GFX_GetViewportSizeInPixels();

			const Fraction viewport_aspect_ratio = {
			        iroundf(viewport_px.w), iroundf(viewport_px.h)};

			const Fraction final_render_aspect_ratio = {
			        final_render_width, final_render_height};
	...
	// Set somne TTF-related values even if the TTF renderer does not intend
	// to override the rendering, but is potentially interested in doing so
	if (vga.draw.ttf.override || vga.draw.ttf.keep_checking_vram_font) {
		vga.draw.ttf.blocks_horizontal = vga.draw.blocks;
		vga.draw.ttf.blocks_vertical   = vga.draw.lines_total /
		                                 vga.draw.address_line_total;
	}
	if (vga.draw.ttf.override) {
		// Cut away all possible remaining lines
		vga.draw.lines_total -= vga.draw.lines_total %
		                        vga.draw.address_line_total;
	}
	...
	if (vga.draw.ttf.override) {
		TTF_CalculateRenderSize(render_width,
		                        render_height,
		                        render_pixel_aspect_ratio);
	} else {
		// Looks like we won't be needing the TTF cache for a while,
		// we can release unneeded renderer cache memory
		TTF_FreeCacheMemory();
	}
...
void VGA_SetupDrawing(uint32_t /*val*/)
{
	...
	if (previous_video_mode != image_info.video_mode ||
		...
		const auto& ttf_override = image_info.video_mode.ttf_override;
		const auto max_width  = ttf_override ? ScalerMaxWidthTtf  : ScalerMaxWidth;
		const auto max_height = ttf_override ? ScalerMaxHeightTtf : ScalerMaxHeight;
		...
		if (image_info.width > max_width || image_info.height > max_height) {
			...
			vga.draw.image_info.width  = std::min(image_info.width, max_width);
			vga.draw.image_info.height = std::min(image_info.height, max_height);
		}
