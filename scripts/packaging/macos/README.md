...
1. Make edits to `packaging/macos/background/background.svg` in Inkscape. Make
   sure you have the font [Open Sans][open-sans] installed.

   [open-sans]: https://fonts.google.com/specimen/Open+Sans

2. Export it twice using the "Page" export mode to `packaging/macos/background`
   at 96 and 192 dpi as `background-1x.png` and `background-2x.png`,
   respectively.

3. In `packaging/macos/background` run `merge-background.sh` to re-generate
...
5. When you're done, copy the hidden `.DS_Store` from the root of the mounted
   volume and overwrite `packaging/macos/DS_Store` with it.
